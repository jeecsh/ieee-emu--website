...
export async function POST(req) {
    try {
      ...
      if (!email || !name || !surname) {
        return NextResponse.json({ error: 'Email, name, and surname are required' }, { status: 400 });
      }
      ...
      if (!brevoResponse.ok) {
        ...
        return NextResponse.json({ message: 'Failed to add contact to Brevo.', error: errorData }, { status: 500 });
      }
